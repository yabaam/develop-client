...
const SuggetionsDropdown: FC<IHomeHeaderProps> = ({ setIsMessageDropdownOpen }): ReactElement => {
  ...
  const selectInboxMessage = async (message: IMessageDocument): Promise<void> => {
    try {
      ...
      navigate(`/suggetions/${lowerCase(chatUsername)}/${message.conversationId}`);
  ...
  return (
    <div className="border-grey border-grey z-20 flex max-h-[470px] flex-col justify-between rounded border bg-white shadow-md">
      <div className="border-grey block border-b px-4 py-2 text-center font-medium text-gray-700">Solicitudes</div>
      <div className="h-96 overflow-y-scroll">
        {conversations.length > 0 ? (
          ...
          <div className="flex h-full items-center justify-center">No solicitudes to show</div>
        )}
      </div>
      <div
        onClick={() => {
...
export default SuggetionsDropdown;
